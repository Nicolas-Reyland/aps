...
pub fn exec_macro_use(macro_str: &str, imports: Option<Vec<&str>>) -> String {
    ...
    for filename in filenames {
        ...
        {
            eprintln!(" invalid #use path (blocked): {}", filename);
            continue;
        }
...
pub fn exec_macro_add(macro_str: &str) -> String {
    ...
    for attr in attr_str.split(",") {
        match attr {
            "a" | "ass" | "associative" => content.push_str(
                "_ :: { (A @ B) @ C = A @ (B @ C) ; }\n"
                    .replace("@", op_str)
                    .as_str(),
            ),
...
pub fn exec_macro_identity(macro_str: &str) -> String {
    ...
    let value = value_str
        .parse::<i32>()
        .expect(format!("Failed to parse <{}> as a number", value_str).as_str());
    format!("_ :: {{ A {} {} = A ; }}\n", op_string, value)
}
