...
fn test_atom_expr_p() {
    assert_eq!(
        atom_expr_p::<ApsParserKind>("A + 0 * (C ^ (2 / Q @ ...)) + $ E * $#B 2"),
        Ok((
            "",
            AtomExpr {
                atoms: vec![
                    ...
                    Atom::Generator(GeneratorExpr {
                        elements: vec![
                            GeneratorElement::GenAtom(Atom::Value('E',),),
                            GeneratorElement::GenOperator(Operator { op: '*' },),
                        ],
                        iterator: Box::new(Atom::Value('B',),),
                    },),
                    Atom::Special('2',),
                ],
                operators: vec![
...
fn test_generator_expr_p() {
    assert_eq!(
        generator_expr_p::<ApsParserKind>("$ * A + B $ # C "),
        Ok((
            "",
            Atom::Generator(GeneratorExpr {
                elements: vec![
                    GeneratorElement::GenOperator(Operator { op: '*' },),
                    GeneratorElement::GenAtom(Atom::Value('A',),),
                    GeneratorElement::GenOperator(Operator { op: '+' },),
                    GeneratorElement::GenAtom(Atom::Value('B',),),
                ],
                iterator: Box::new(Atom::Value('C',)),
            },),
        ),)
    )
}
...
fn test_fn_call_p() {
    assert_eq!(
        fn_call_p::<ApsParserKind>("exp(A ^ 2 - 4) "),
        Ok((
            "",
            Atom::FunctionCall((
                "exp".to_string(),
                AtomExpr {
                    atoms: vec![Atom::Value('A',), Atom::Special('2',), Atom::Special('4',),],
                    operators: vec![Operator { op: '^' }, Operator { op: '-' },]
                }
            ))
        ))
